...
KIND_CLUSTER ?= hackaway-repo-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name hackaway-repo-builder
	$(CONTAINER_TOOL) buildx use hackaway-repo-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm hackaway-repo-builder
	rm Dockerfile.cross
...
kind-setup: ## Create a kind cluster and install CRDs.
	...
	@$(KUBECTL) create namespace clawbernetes --dry-run=client -o yaml | $(KUBECTL) apply -f -
	$(MAKE) install
